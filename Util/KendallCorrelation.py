'''
    ...
    Calculates the Kendall Tau correlation coefficient between the target and all features.
...
class KendallCorrelation:
    ...
    def __init__(self, data_root):
        ...
        self._kendall_results = {}
        self._timer_start = 0
        self._timer_end = 0


    def set_kendall_calc(self):

        print("Beginning Kendall Tau Calc...")
        ...
        for i in self._feature_list:
            ...
            value: float = corr
            loop_dict = {key: value}
            self._kendall_results.update(loop_dict)

        self._set_execution_timer()

        print(f"\nKendall Tau completed in : {self._get_execution_timer()} seconds.")
