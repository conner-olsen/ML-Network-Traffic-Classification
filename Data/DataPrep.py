...
class DataPrep:
    ...
    def __init__(self, raw_data_path, data_root, convert_strings=True):
        ...
        self._train_filename_resamp = self._data_root + "training/CIDDS_Internal_train_resample.csv"
        self._test_filename_resamp = self._data_root + "testing/CIDDS_Internal_test_resample.csv"
    ...
    def split_data(self, resample=True):
        ...
        if (self._convert_strings == False):
        ...
        if (resample != True):     #don't resample data
    ...
    def _calulate_bytes(self, mega):
        bytes = int(float(mega.split()[0]) * 1000000)
        return bytes
